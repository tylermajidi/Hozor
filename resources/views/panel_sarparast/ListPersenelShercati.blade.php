...
@section('PageTitle') میزکار سرپرست (اضافه کاری شرکتی)  @stop
@section('NameUser'){{session('usernum')}}  @stop
@section('PageHeader') صفحه ثبت اضافه کاری شرکتی  @stop
@section('PageHeaderInfo')@stop
@section('PageAddress') اضافه کاری شرکتی  @stop
@section('PageCardName') لیست پرسنل @stop
@section('PageCardInfo')ثبت  اضافه کاری شرکتی پرسنل@stop
...
@section('PageCardData')
    ...
    <div class="container ">
        ...
        <div class="d-flex justify-content-around  ">
                ...
                <div class="input-group  float-right  col-md-8 col-sm-12 ">
                    ...
                    <form id="formserchresualt"  method="POST" action="/sarparast/PageListPersenelShercati2"  class=" ">
                        @csrf
        ...
        <table id="listperseneltable"   class="table table-striped table-bordered second sm-font-10" style=" text-align: center; ">
            ...
            @foreach($persenellist  as $index=> $row)
                <tr >
                    ...
                    @if($row->ezafekar_shercat==0)
                    <td ><input name="saat" id="saat" type="number" value="" style="width:75px;text-align: center " placeholder="ساعت" ></td>
                    @else
                        <td ><input name="saat" id="saat" type="number" value="{{$row->ezafekar_shercat}}" style="width:75px;text-align: center " placeholder="ساعت" ></td>
    ...
    <form id="formsaveshercati" action="/sarparast/addٍٍEzafeKarSherkati"  method="POST">
        @csrf
        ...
        <button  type="button" onclick="GetEzafeShercat();saveshercatimessage()" class="btn-outline-success float-left m-l-10 " style="padding: 10px" >ذخیره فرم</button>
    </form>
...
@section('PageScript')
    ...
    <script>
        function saveshercatimessage() {


            swal({title: "ذخیره اضافه کاری شرکتی", text: "آیا مایل به ذخیره لیست اضافه کار شرکتی هستید؟",
                icon: "warning",
                buttons: true,
                warningMode :true,
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        document.getElementById("formsaveshercati").submit();
